// The GCS object-store backend. It lives in its own module for one reason: the
// Google Cloud SDK. See docs/src/guides/go-modules.md for the layout rules, the
// no-go.work decision and the tagging policy; doc.go next door records why this
// backend takes an SDK at all when the rest of the tree is raw net/http, and
...
require (
	...
	// Pinned to the newest core release rather than a v0.0.0 placeholder, so a
...
// CI and local development build this backend against the working tree, so a
// change to pkg/engine/objectstore that breaks an out-of-module implementer
