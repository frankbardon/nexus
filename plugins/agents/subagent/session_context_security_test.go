...
// (react/orchestrator/subagent) drive unexported plugin state — this
// package's own p.session, orchestrator's p.originalInput +
// sendSynthesizeRequest — that is only reachable from inside each plugin's
// own package; see this story's FOLLOWUPS for the concrete reason a single
// file isn't possible. All four are deliberately named identically (Go test
// names are scoped per package) and use the identical label pair
// ("team"/"acme" general, "_principal_id"/"user-42" reserved) so the four
// runs read as one table:
//
...
//   - plugins/agents/subagent/session_context_security_test.go (this file)
//   - plugins/workflows/icm/runtime/session_context_security_test.go (OperatorTemplateCtx + PayloadBuilder)
//
// A future change that lets a reserved key leak into subagent's
// <session_context> block, prepended ahead of the configured systemPrompt
// (E3-S3), breaks THIS test — unambiguously, by name, as a security
...
func TestSessionContext_ReservedKeyNeverLeaksIntoPrompt(t *testing.T) {
	...
	if strings.Contains(sys, "user-42") || strings.Contains(sys, "_principal_id") {
		t.Errorf("SECURITY REGRESSION: subagent system prompt leaked reserved label: %s", sys)
	}
}
