// Package jina implements the search.reranker capability backed by the Jina
// AI Reranker API. Direct HTTP, no SDK.
package jina
...
const (
	pluginID     = "nexus.rag.reranker.jina"
	pluginName   = "Jina Reranker"
	version      = "0.1.0"
	apiURL       = "https://api.jina.ai/v1/rerank"
	defaultModel = "jina-reranker-v2-base-multilingual"
)

// Plugin advertises search.reranker via the Jina Reranker API.
type Plugin struct {
	bus    engine.EventBus
	logger *slog.Logger
...
func (p *Plugin) Capabilities() []engine.Capability {
	return []engine.Capability{{
		...
		Description: "Cross-encoder reranker via the Jina Reranker API.",
	}}
}
...
func (p *Plugin) Init(ctx engine.PluginContext) error {
	...
	if p.apiKey == "" {
		return fmt.Errorf("jina reranker: no API key configured (set api_key or JINA_API_KEY env var)")
	}
...
func (p *Plugin) rerank(query string, docs []events.RerankDoc, model string, topN int) ([]events.RerankResult, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("jina returned HTTP %d: %s", resp.StatusCode, truncate(string(respBody), 300))
	}
