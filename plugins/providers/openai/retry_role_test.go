...
func TestValidateRoleRetry_UnknownKeyFailsInitNamingTheRole(t *testing.T) {
	...
	err := validateRoleRetry(models, nil, silentLogger())
	if err == nil {
...
func TestValidateRoleRetry_WrongTypeFailsInit(t *testing.T) {
	...
	if err := validateRoleRetry(models, nil, silentLogger()); err == nil {
...
func TestValidateRoleRetry_ForeignEntriesAndNilRegistry(t *testing.T) {
	...
	if err := validateRoleRetry(nil, nil, silentLogger()); err != nil {
...
func TestValidateRoleRetry_WalksTheWholeChain(t *testing.T) {
	models := engine.NewModelRegistry(map[string]any{
		"default": "balanced",
		"balanced": []any{
			map[string]any{"provider": pluginID, "model": "gpt-4o"},
			map[string]any{"provider": pluginID, "model": "gpt-4o-mini", "retry": map[string]any{"backoff": 7}},
		},
	})
...
func retryModels(entries map[string]map[string]any) *engine.ModelRegistry {
	...
	for role, block := range entries {
		cfg := map[string]any{"provider": pluginID, "model": "gpt-4o", "max_tokens": 2048}
		if block != nil {
			cfg["retry"] = block
		}
		raw[role] = cfg
...
func retryPlugin(t *testing.T, models *engine.ModelRegistry, block map[string]any) *Plugin {
	...
	return &Plugin{logger: silentLogger(), models: models, retry: rc, retryRaw: rawRetryBlock(cfg)}
}
...
func TestRoleRetry_RecoveredAfterModelRewrite(t *testing.T) {
	...
	rc := resolveRetryFor(p, events.LLMRequest{Role: "quick", Model: "gpt-4o-mini"})
	if rc.MaxRetries != 1 {
		t.Fatalf("max_retries = %d, want the role's 1", rc.MaxRetries)
	}
}
...
func TestRoleRetry_FallbackStampWins(t *testing.T) {
	models := engine.NewModelRegistry(map[string]any{
		"default": "balanced",
		"balanced": []any{
			map[string]any{"provider": pluginID, "model": "gpt-4o", "retry": map[string]any{"max_retries": 1}},
			map[string]any{"provider": pluginID, "model": "gpt-4o-mini", "retry": map[string]any{"max_retries": 9}},
		},
	})
	...
	req := events.LLMRequest{Role: "balanced", Model: "gpt-4o-mini"}
...
func TestRoleRetry_FanoutLegStampWins(t *testing.T) {
	models := engine.NewModelRegistry(map[string]any{
		...
		"balanced": map[string]any{
			...
			"providers": []any{
				map[string]any{"provider": pluginID, "model": "gpt-4o"},
				map[string]any{"provider": pluginID, "model": "gpt-4o-mini", "retry": map[string]any{"max_retries": 0}},
			},
		},
	})
	p := retryPlugin(t, models, map[string]any{"max_retries": 5})
