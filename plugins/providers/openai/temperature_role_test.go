...
// wireTemperature runs the step handleRequest does before building the body and
// reports what `temperature` ended up as. A false second return means no
...
// the default role's entry — the same rule max_tokens follows.
...
func TestRoleTemperature_StampedEntryWinsOverTheRole(t *testing.T) {
	models := engine.NewModelRegistry(map[string]any{
		"default": "balanced",
		"balanced": []any{
			map[string]any{"provider": pluginID, "model": "gpt-4o", "temperature": 1},
			map[string]any{"provider": pluginID, "model": "gpt-4o-mini", "temperature": 0.1},
		},
	})
	p := tempPlugin(models)
...
// handleRequest's model-resolution branches never run. The role's temperature
...
func TestRoleTemperature_RecoveredAfterModelRewrite(t *testing.T) {
	...
	got, ok := wireTemperature(t, p, events.LLMRequest{Role: "quick", Model: "gpt-4o"})
	if !ok || got != 0.2 {
