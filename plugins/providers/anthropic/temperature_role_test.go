...
// the request body. buildRequestBody has always copied req.Temperature onto the
// wire; nothing proved a plain single-entry role's `temperature:` ever became
// that value.
//
...
// entry onto the request, then build the body — and reports what `temperature`
// ended up as. A false second return means no temperature key was emitted.
//
...
// tempPlugin builds the minimum Plugin buildRequestBody runs against, with
// thinking off so applyThinking does not strip the field back out.
...
func TestRoleTemperature_StampedEntryWinsOverTheRole(t *testing.T) {
	models := engine.NewModelRegistry(map[string]any{
		"default": "balanced",
		"balanced": []any{
			map[string]any{"provider": pluginID, "model": "claude-opus-4-7", "temperature": 1},
			map[string]any{"provider": pluginID, "model": "claude-haiku-4-5", "temperature": 0.1},
		},
	})
	p := tempPlugin(models)
...
func TestRoleTemperature_RecoveredAfterModelRewrite(t *testing.T) {
	...
	got, ok := wireTemperature(t, p, events.LLMRequest{Role: "quick", Model: "claude-haiku-4-5"})
	if !ok || got != 0.2 {
