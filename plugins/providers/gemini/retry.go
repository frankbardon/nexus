...
func validateRetryBlock(raw map[string]any) error {
	...
	if len(unknown) > 0 {
		...
		return fmt.Errorf("gemini: retry block has unknown key(s) %s; accepted keys are %s",
			strings.Join(unknown, ", "), retryBlockKeyList)
	}

	if v, present := raw["enabled"]; present {
		if _, ok := v.(bool); !ok {
			return fmt.Errorf("gemini: retry.enabled must be a bool, got %T", v)
		}
	}
	...
	if v, present := raw["max_retries"]; present {
		n, ok := retryNumber(v)
		if !ok {
			return fmt.Errorf("gemini: retry.max_retries must be an int, got %T", v)
		}
		if n < 0 {
			return fmt.Errorf("gemini: retry.max_retries must not be negative, got %v", v)
		}
	}
	if v, present := raw["multiplier"]; present {
		n, ok := retryNumber(v)
		if !ok {
			return fmt.Errorf("gemini: retry.multiplier must be a number, got %T", v)
		}
		if n < 0 {
			return fmt.Errorf("gemini: retry.multiplier must not be negative, got %v", v)
		}
	}
	for _, key := range []string{"initial_delay", "max_delay", "backoff"} {
		if v, present := raw[key]; present {
			if _, ok := v.(string); !ok {
				return fmt.Errorf("gemini: retry.%s must be a string, got %T", key, v)
			}
		}
	}
	if v, present := raw["statuses"]; present {
		list, ok := v.([]any)
		if !ok {
			return fmt.Errorf("gemini: retry.statuses must be a list of ints, got %T", v)
		}
		for _, s := range list {
			if _, ok := retryNumber(s); !ok {
				return fmt.Errorf("gemini: retry.statuses entries must be ints, got %T", s)
			}
		}
	}
	return nil
...
func parseRetryConfig(cfg map[string]any, logger *slog.Logger) (retryConfig, error) {
	...
	if v, ok := retryCfg["backoff"].(string); ok {
		...
		default:
			if logger != nil {
				logger.Warn("gemini: retry.backoff is not a known strategy; keeping the default",
					"backoff", v,
...
func retryDuration(raw map[string]any, key string, def time.Duration, logger *slog.Logger) time.Duration {
	...
	if err != nil {
		if logger != nil {
			logger.Warn("gemini: retry duration is not a Go duration; keeping the default",
				"key", key,
				"value", v,
				"default", def,
			)
...
func (p *Plugin) resolveRetry(req events.LLMRequest) retryConfig {
	...
	if err != nil {
		...
		logger.Warn("gemini: ignoring an invalid per-request retry block; using the plugin-level configuration",
			"role", req.Role,
			"error", err,
		)
		return p.retry
