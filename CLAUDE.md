...
- **Request-header convention** (`pkg/nexusheaders/`) — the single definition of the `X-Nexus-*` request-header namespace: `Prefix`, `Name` (normalize one field name), `Extract` (pull the namespace off an `http.Header`, bounded and control-character-stripped). Every web-facing IO transport — `nexus.io.agui`, `nexus.io.a2a`, `nexus.io.browser`, `nexus.io.realtime`, and `nexus.io.broker` via the gateway, which the instance never sees the client request behind: the broker sets them on the `input` payload it builds on its A2A surface, and on the opaque client-stream pipe sends a broker-authored `client.headers` payload ahead of each client IO frame carrying the handshake's headers, which MERGE with the per-turn `headers` a client may set on its own `input` payload — handshake wins per key, since it comes from the HTTP hop a reverse proxy controls, so fixed values go on the handshake and varying ones on the turn; the per-turn half is normalized and bounded by `nexusheaders.Sanitize`, and identity is never merged — carries them onto `events.UserInput.Headers` (schema v3) AND binds them as reserved `_header.<name>` session labels via `SessionWorkspace.SetRequestHeaders`, so a plugin outside the `io.input` path reads them with `session.RequestHeaders()`. Values are **client-controlled and unauthenticated** — a data-injection mechanism, NOT an auth surface: the reserved namespace keeps them unforgeable by plugins and invisible to prompts, and surfacing one to the model is an opt-in allowlist (`nexus.system.dynvars.request_headers`). Replaced wholesale every turn, so a turn that drops a header never inherits the last one's value. They are persisted into `metadata/session.json` (and the object store), so a secret must never ride one — pass a subject and exchange it downstream. Verified identity is the sibling seam and the one to gate on: `engine.ReservedPrincipalIDKey` (`_principal_id`) with `SessionWorkspace.SetPrincipalID`/`PrincipalID`, bound by `nexus.io.agui` from its auth chain and by `nexus.io.broker` from the principal the broker resolved and forwards on the same payload as the headers (separate field, announcement wins over a client-set one). Operators should network-lock the broker's client endpoints to their own gateway. See `docs/src/guides/request-headers.md`.
...
- **Provider round-trip metadata** (`pkg/roundtrip/`) — the one allowlist of `LLMResponse.Metadata` keys (`thinking_blocks`, `gemini_thought_signatures`, `openai_reasoning_items`) that must ride the assistant `Message` a later request replays, or the provider rejects that request outright — or, in OpenAI's case, silently loses its reasoning across a tool round. Every history builder calls `ForwardMessageMetadata` — the memory plugins for persisted history, and the agent loops that keep their own in-process history (`pkg/delegate`, `plugins/agents/subagent`, `plugins/agents/planexec`, `plugins/agents/orchestrator`), which no memory plugin covers. It was `plugins/memory/internal/roundtrip` until those loops turned out to need it too: a delegated posture that called one tool and asked again sent Gemini a `functionCall` with no `thoughtSignature` and got `400 INVALID_ARGUMENT` every time. The Gemini key is a string literal here and an unexported constant in the provider — coupled by convention, change one and change the other.
...
- **OpenAI Responses conformance corpus** (`pkg/openaiconform/`) — test-only shared corpus pinning the TWO independent `/v1/responses` serializers and reply parsers — `plugins/providers/openai` and `plugins/llm/batch` — against one set of JSON vectors. They cannot share code (the provider's builders are unexported methods on plugin state, and reaching across would be a plugin-to-plugin call), so this replaces the compile coupling they give up. Each vector carries the WHOLE expected body, so a **key-set** divergence (one side gains or loses a field) fails, not just a changed value; settled invariants are asserted by name (`store: false`, explicit `strict: false` per function tool, `max_output_tokens`, `input`, `text.format`, `model` present in Azure modes, `reasoning` object applied last); legitimately-differing fields are the explicit `RequestDivergences`/`ReplyDivergences` allowlist, enforced in BOTH directions. Drivers: `plugins/{providers/openai,llm/batch}/conform_test.go`. Adding a vector is a JSON file. See `docs/src/guides/plugin-contracts.md`.
...
`before:llm.stream.chunk` is its streaming counterpart, and it exists because a veto at end-of-stream cannot unsay rendered text. Every text delta goes through `engine.StreamPublisher` (`pkg/engine/llm_stream.go`), which offers it as an `*events.StreamSegment` before any of it reaches the bus — so handlers pass, redact, **suspend** (`Hold` withholds a trailing fragment and re-offers it with the next delta), or block. Because withheld text is never emitted, a block is prevention rather than retraction, and the guarantee holds for transports with no retraction verb. Detection does not depend on the hold: handlers scan the cumulative turn and act on where a match *ends*, so the hold bounds the residue rather than doing the detecting. `llm.stream.hold` announces both edges of a suspension and `llm.stream.retract` disowns a blocked prefix; every IO transport maps both at whatever fidelity its protocol allows. **Every producer of an `llm.stream.chunk` must publish via a `StreamPublisher` rather than `bus.Emit("llm.stream.chunk", ...)`.** A plugin gating `before:llm.response` without gating `before:llm.stream.chunk` costs the deployment streaming: the engine reads declared `Subscriptions()` at boot and clears `Stream` on outbound requests (`core.streaming.response_gate_policy`).
