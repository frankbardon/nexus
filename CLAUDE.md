...
- **Session broker** (`cmd/nexus-broker/`) — Standalone HTTP/WS gateway service (NOT a plugin) that fronts OS-isolated `nexus` instances behind one ingress: clients `claim` a lease, the broker cold-spawns a `nexus` subprocess that dials back via the `nexus.io.broker` plugin, and instances `release` on demand/idle/crash with sessions persisted on disk. It also fronts an **A2A ingress**: each `agents:` profile publishes an Agent Card and both A2A bindings under `/agents/<name>/`, and the broker translates between A2A and the instance IO envelope — the second of the two A2A mappings judged by `pkg/a2a/a2aconform`. See `docs/src/guides/session-broker.md`.
