...
var ErrNotOnGCE = errors.New("gcemeta: not running on GCE; there is no metadata server to ask")
...
func ProjectID(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("gcemeta: reading the project ID from the GCE metadata server: %w", err)
	}
	if id == "" {
		return "", fmt.Errorf("gcemeta: the GCE metadata server returned an empty project ID")
	}
	return id, nil
}
...
func Zone(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("gcemeta: reading the zone from the GCE metadata server: %w", err)
	}
	if zone == "" {
		return "", fmt.Errorf("gcemeta: the GCE metadata server returned an empty zone")
	}
	return zone, nil
}
...
func Region(ctx context.Context) (string, error) {
	...
	if region == "" {
		return "", fmt.Errorf("gcemeta: the GCE metadata server returned zone %q, which is not of the form <region>-<letter>", zone)
	}
	return region, nil
}
