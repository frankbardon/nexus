...
func TestDefaultConfig_ObjectStoreDisabled(t *testing.T) {
	c := DefaultConfig()
	if c.Core.Sessions.ObjectStore.Enabled() {
		t.Fatal("DefaultConfig enables the object store; the default path must stay local-only")
	}
	if c.Core.Sessions.ObjectStore != (objectstore.Config{}) {
		t.Errorf("DefaultConfig object store = %+v, want the zero value", c.Core.Sessions.ObjectStore)
	}
}
...
func TestLoadConfig_NoObjectStoreBlockIsZeroValued(t *testing.T) {
	...
	if cfg.Core.Sessions.ObjectStore != (objectstore.Config{}) {
		t.Errorf("object store = %+v, want zero value for a config that never mentions it", cfg.Core.Sessions.ObjectStore)
	}
}
...
`))
	...
	got := cfg.Core.Sessions.ObjectStore
...
`))
	...
	if got := cfg.Core.Sessions.ObjectStore.FailurePolicy; got != objectstore.FailurePolicyDegrade {
...
`))
	...
	for _, want := range []string{"core.sessions.object_store.backend", "gcs"} {
...
func TestLoadConfig_ObjectStoreKeyErrorsNameTheKey(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			name:    "missing bucket",
			yaml:    "core:\n  sessions:\n    object_store:\n      backend: fake-errs\n",
			wantKey: "core.sessions.object_store.bucket",
		},
		{
			name:    "invalid failure policy",
			yaml:    "core:\n  sessions:\n    object_store:\n      backend: fake-errs\n      bucket: b\n      failure_policy: explode\n",
			wantKey: "core.sessions.object_store.failure_policy",
		},
		{
			name:    "prefix with a leading slash",
			yaml:    "core:\n  sessions:\n    object_store:\n      backend: fake-errs\n      bucket: b\n      prefix: /leading\n",
			wantKey: "core.sessions.object_store.prefix",
		},
		{
...
`))
	...
	if got := cfg.Core.Sessions.ObjectStore.CredentialsFile; got != want {
...
`))
	...
	if got := cfg.Core.Sessions.ObjectStore.Prefix; got != "~tenant/nexus" {
