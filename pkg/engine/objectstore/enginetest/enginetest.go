// Package enginetest holds the shared kill-and-resume suite: a real
// *engine.Engine, driven through a real object-store backend, killed, and
// resumed on a clean filesystem.
//
// # Why this is a second suite and not part of objectstoretest
//
// objectstoretest answers "is this a conformant Backend?" from the outside,
// with nothing but the interface. This one answers the question the effort
// actually exists for -- "does a session survive the death of the process that
// produced it?" -- and it can only be answered by booting the engine, so this
// package imports pkg/engine.
//
// That import is why it is a package of its own rather than more files in
// objectstoretest. pkg/engine's own tests are `package engine` (internal) and
// import objectstoretest for its Memory backend; if objectstoretest imported
// pkg/engine back, that test binary would not build. Keeping the
// engine-importing half in a sibling package keeps both directions legal.
//
// # Why it is exported at all
//
// The same reason objectstoretest is. A backend may live in a module this
// repository never sees, and "it passed the interface conformance suite" does
// not mean a session resumes from it: the WAL checkpoint has to survive a real
// upload, Flush has to be durable outside the process, and the manifest has to
// name a restorable object set. Two in-repo backends run this suite -- S3
// against MinIO, GCS against fake-gcs-server -- and they run the same one on
// purpose. Two hand-written copies of a 500-line kill-and-resume choreography
// diverge the first time either is fixed, and a divergence would show up as one
// backend quietly proving less than the other rather than as a failure.
//
// # What a caller has to supply
//
// Everything here is store-independent except four things: how to register a
// factory, how to get an empty container, how to read that container back
// WITHOUT the backend under test, and which config keys the backend needs.
// Store is those four things and nothing else.
//
// # What this suite does not claim
//
// Not IAM, in either cloud. Both suites run against an emulator, and no
// emulator reproduces IRSA, EKS Pod Identity, GKE Workload Identity,
// Application Default Credentials resolution or Workload Identity Federation.
// Those are the legs that justify taking a vendor SDK at all, and they stay
// covered by a documented manual check, not by this.
package enginetest
...
// Store is the store-specific half of the suite: the things only a backend's
// own ecosystem can do, expressed so the choreography below does not have to
// know which cloud it is talking to.
//
// Every method takes *testing.T rather than returning an error, matching
// objectstoretest: a helper that fails should say which store and which
// operation, at the line it happened, and a caller who has to check four error
// returns writes four different messages for the same failure.
type Store interface {
	// Name is what failure messages call this store -- "MinIO",
	// "fake-gcs-server". It appears in assertions an implementer will read
	// without knowing which suite produced them.
	Name() string

	// Register registers an objectstore.Factory under a name unique to the
	// running test and returns that name, for an engine to select from YAML.
	//
	// A per-call name rather than the backend's real one: a single case needs
	// two independently controllable backends (the mid-flush case arms one and
	// leaves the other alone), and objectstore's registry is process-global, so
	// sharing the real name would make the cases order-dependent.
	//
	// wrap, when non-nil, interposes on the backend the engine will use. That
	// is the entire mechanism for the mid-flush case: there is no seam inside a
	// backend for "die here", and adding one would be a production change made
	// to satisfy a test.
	//
	// Anything that must run on the test goroutine -- t.Setenv for the
	// credentials the backend will resolve, for instance -- belongs here rather
	// than inside the factory, which runs during Boot.
	Register(t *testing.T, label string, wrap func(objectstore.Backend) objectstore.Backend) string

	// NewBucket creates an empty container and registers its teardown on t.
	// A container per case, never a shared one with a per-case prefix: the
	// suite asserts on the whole key set the engine wrote.
	NewBucket(t *testing.T) string

	// ObjectStoreYAML renders the body of the core.object_store block for a
	// bucket and a registered backend name, indented four spaces so it nests
	// under `core:` -> `object_store:`.
	//
	// The suite does not build this itself because the two backends need
	// different keys: S3 signs over a region, GCS has none and warns if one is
	// given. Rendering YAML rather than an *engine.Config is not a stylistic
	// choice -- newFromConfig is unexported, so engine.NewFromBytes is the only
	// cross-module way in, and it is the path an operator's config takes,
	// including the object-store block's own validation.
	ObjectStoreYAML(backendName, bucket string, policy objectstore.FailurePolicy) string

	// Keys lists every object key in the container, using a client that is NOT
	// the backend under test. Assertions made through the backend that wrote
	// the objects would agree with themselves.
	Keys(t *testing.T, bucket string) []string

	// Get downloads one object with that same independent client.
	Get(t *testing.T, bucket, key string) []byte

	// Exists reports whether one object is present.
	Exists(t *testing.T, bucket, key string) bool
}

// resumePluginID is the fake plugin whose session-scope SQLite store stands in
// for real per-plugin state.
//
// Session scope specifically: it is the only scope that lives inside the
// session tree, and therefore the only one the session snapshot carries. The
// same ID pkg/engine's own resume suite uses, so a failure here and a failure
// there name the same thing.
const resumePluginID = "nexus.test.resume"

// RunResumeSuite runs the full cycle against store.
//
// Two cases, both of which kill an engine rather than stopping it, because the
// property under test is what a turn-boundary snapshot alone can restore.
// Nothing here uses t.Parallel: a caller may be pointed at a shared emulator,
// and a suite that is only correct when run serially is better than one that is
// intermittently wrong.
func RunResumeSuite(t *testing.T, store Store) {
	t.Helper()
	if store == nil {
		t.Fatal("enginetest: RunResumeSuite called with a nil Store")
	}
	t.Run("KillAndResumeRestoresIdenticalSessionState", func(t *testing.T) {
		runKillAndResume(t, store)
	})
	t.Run("MidFlushKillRestoresTheCommittedGeneration", func(t *testing.T) {
		runMidFlushKill(t, store)
	})
}

// ---------------------------------------------------------------------------
// Wiring an engine to a real store
// ---------------------------------------------------------------------------

// newEngine builds an engine rooted at dir and pointed at a registered backend.
func newEngine(t *testing.T, store Store, backendName, bucket string, policy objectstore.FailurePolicy, dir string) *engine.Engine {
	t.Helper()
...
`,
		...
		YAMLString(dir),
		strings.TrimRight(store.ObjectStoreYAML(backendName, bucket, policy), "\n"),
	)
...
// YAMLString renders a value as a YAML double-quoted scalar, for a Store
// building its ObjectStoreYAML block.
//
// strconv.Quote's escaping is a subset of YAML's for the ASCII these values
// are (temp paths, backend names, URLs), and it means a t.TempDir() containing
// a character YAML would otherwise treat as syntax cannot silently produce a
// different config.
...
// findKeyWithSuffix returns the single key in the container ending in suffix.
//
// The engine's object-key scheme is unexported, and duplicating it here would
// mean this suite passes or fails on a copy of a constant rather than on what
// the engine did. Locating a key by the tail it must have -- the
// session-relative path -- keeps that dependency observational.
func findKeyWithSuffix(t *testing.T, store Store, bucket, suffix string) string {
	t.Helper()
	all := store.Keys(t, bucket)
	var found []string
	for _, k := range all {
	...
	if len(found) != 1 {
		t.Fatalf("want exactly one object ending in %q, found %v (%s holds %v)",
			suffix, found, store.Name(), all)
	}
	return found[0]
}
...
// modernc.org/sqlite, reached here through pkg/engine.
...
// runToKill boots a session against the store, fills it with every category of
// state the effort names, completes one turn, writes some more, and then
...
func runToKill(t *testing.T, store Store, backendName, bucket string) *killedSession {
	t.Helper()

	eng := newEngine(t, store, backendName, bucket, objectstore.FailurePolicyStrict, t.TempDir())
	...
	// blobs.New over BlobsDir, matching pkg/engine's own resume suite: it is
	// the constructor every blob-producing plugin uses, and going through it
	// rather than SessionWorkspace.BlobStore keeps the write-through worker out
	// of the picture so the turn-boundary snapshot is the only thing that can
	// have put this blob in the bucket.
	...
	// removes the one way this suite could lie about a real process death: an
	// engine left running keeps a heartbeat and (under degrade) a retry worker
	// writing objects into the container the caller's teardown is trying to
	// empty, and the container delete then fails intermittently as non-empty.
	eng.Abandon()
	return k
}

// runKillAndResume is the headline case: run a session against a real store in
// another process, complete a turn, kill the process, boot a fresh engine over
// a clean filesystem, and get the same session back -- history, artifacts,
// blobs and per-plugin SQLite alike.
func runKillAndResume(t *testing.T, store Store) {
	bucket := store.NewBucket(t)
	backendName := store.Register(t, "resume", nil)

	k := runToKill(t, store, backendName, bucket)
	...
	freshRoot := t.TempDir()
	eng := newEngine(t, store, backendName, bucket, objectstore.FailurePolicyStrict, freshRoot)
	...
	// --- per-plugin SQLite, as the store is holding it -------------------
	//
	...
	// open; this pulls the object straight out of the container with a client
	// that is not the backend under test and asserts THOSE bytes are a valid,
	// queryable database holding all 500 rows. That is what proves the session
	// snapshot's wal_checkpoint(TRUNCATE) + VACUUM INTO survived a real upload
	// rather than an in-process handoff -- an uncheckpointed upload lands a
	// main database file that opens fine and is missing most of its content,
	// which is exactly the shape this catches.
	dbKey := findKeyWithSuffix(t, store, bucket, "/plugins/"+resumePluginID+"/store.db")
	label := "the store.db object in " + store.Name()
	remoteRows := openSQLiteBytes(t, store.Get(t, bucket, dbKey), label)
	if len(remoteRows) != len(k.dbRows) {
		t.Errorf("%s holds %d rows, want %d — the WAL checkpoint did not survive the upload",
			label, len(remoteRows), len(k.dbRows))
	}
	for key, want := range k.dbRows {
		if remoteRows[key] != want {
			t.Errorf("%s has row %q = %q, want %q", label, key, remoteRows[key], want)
		}
	}
...
// objects of the dead generation are in the container -- both backends are
// synchronous, so they are durable the moment Put returns -- and the manifest
...
// this case from being a copy of the engine's unexported key scheme.
...
// runMidFlushKill is the mid-flush criterion, asserted at the boundary the
// design actually holds rather than the stronger one the story text reads as.
//
// # The boundary
//
...
//     is not materialised.
//   - Orphaned objects are left in the container, not deleted. This effort
//     never removes remote data; reclamation is the operator's.
//
...
// wire, a real container and a real SQLite file in between, which is where an
// argument that "the store would somehow keep the old bytes" would be exposed.
// The fix would be per-generation object keys -- a second full copy of every
// session in the bucket for ever -- which the snapshot design costed and
// rejected.
func runMidFlushKill(t *testing.T, store Store) {
	bucket := store.NewBucket(t)

	// Two registrations over one container. The dying engine's backend stays
	...
	dyingName := store.Register(t, "dying", func(b objectstore.Backend) objectstore.Backend {
		dying = &interruptedBackend{Backend: b}
		return dying
	})
	var resuming *interruptedBackend
	resumingName := store.Register(t, "resuming", func(b objectstore.Backend) objectstore.Backend {
		resuming = &interruptedBackend{Backend: b}
		return resuming
	})

	// Degrade, not strict: strict would raise core.error and close the turn
	// gate, and this case is about what the container is left holding.
	eng := newEngine(t, store, dyingName, bucket, objectstore.FailurePolicyDegrade, t.TempDir())
	if err := eng.Boot(context.Background()); err != nil {
		t.Fatalf("Boot: %v", err)
	}
	...
	// The session's key prefix, learned from the container rather than rebuilt
	// from the engine's unexported scheme.
	gen1Key := findKeyWithSuffix(t, store, bucket, "/files/generation-1.md")
	...
	// *** kill *** -- everything below reads the container, never this engine.
	...
	// nothing is still uploading into the container the caller's teardown has
	// to empty. The backend stays armed for the same reason it always did -- a
	// second line of defence costs nothing.
	eng.Abandon()

	// The state the kill left in the store, measured with the independent
	// client: the tree of the dead generation is there, its commit is not. If
	// either half of this is wrong the assertions below are not testing a
	// mid-flush kill.
	orphanKey := treePrefix + "files/generation-2.md"
	if !store.Exists(t, bucket, orphanKey) {
		t.Fatalf("%q never reached the store; the interruption happened before the tree upload, "+
			"not partway through it", orphanKey)
	}
	label := "the store.db object in " + store.Name()
	if remote := openSQLiteBytes(t, store.Get(t, bucket, dbKey), label); remote["generation"] != "2" {
		t.Fatalf("%s reads generation %q, want %q — "+
			"the interrupted snapshot did not overwrite it in place, so the boundary below is not being exercised",
			label, remote["generation"], "2")
	}

	// --- a fresh host picks the session up -------------------------------
	freshRoot := t.TempDir()
	resumed := newEngine(t, store, resumingName, bucket, objectstore.FailurePolicyDegrade, freshRoot)
	...
	if rows := readSQLiteKV(t, restoredDB, "the restored store.db"); rows["generation"] != "2" {
		t.Errorf("restored store.db says generation %q, want %q — "+
			"this case pins the in-place overwrite boundary, not a guarantee", rows["generation"], "2")
	}

	// Orphans are left in the store. This effort never deletes remote data.
	if !store.Exists(t, bucket, orphanKey) {
		t.Errorf("hydration deleted the orphaned object %q from the store", orphanKey)
	}
	if resuming == nil {
