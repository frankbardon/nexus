...
//
// Use case: regenerate a derived view from the journal alone — e.g.
// rebuild a per-plugin JSONL file after it was deleted, or feed a
// projection plugin that did not exist when the original session ran.
// Pairs with Writer.SubscribeProjection — same handler signature, same
// filter semantics — so the same code path drives both live and
// post-mortem projection.
//
