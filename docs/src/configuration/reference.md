...
# broker.yaml
listen_addr: ":8080"
log_level: "info"             # trace, debug, info, warn, error; see below
...
| `log_level`          | string   | `info`   | This **process's own** slog level: `trace`, `debug`, `info`, `warn` or `error`. Parsed through the **same** `pkg/engine.ParseLogLevel` the engine's `core.log_level` uses (see [`core`](#core) above), so the two vocabularies never drift apart; an unrecognized value is **not** a boot failure and falls back to `info`, exactly as the engine does. **Boot-only, not reloadable**: `main.go` builds the process's `slog.TextHandler` from this value once, before the `SIGHUP` reload machinery exists to have anything to swap it on, and a plain `slog.NewTextHandler` carries no live level knob. A reloaded file that changes it is reported and ignored — see [Reloadable keys (`SIGHUP`)](#reloadable-keys-sighup) below. |
...
| `log_level` | **No** | Builds the process's own `slog` handler in `main.go` before this reload machinery exists, and a plain `slog.NewTextHandler` carries no live level knob to swap once built. |
